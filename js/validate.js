...
if ($password.match(/(.*[!,%,&,@,#,$,^,*,?,_,~].*[!,%,&,@,#,$,^,*,?,_,~])/)) strength += 1
	...
	if($checkAllData >= 10)
	{
		var datastr = 'firstname='+$('#firstname').val() + '&lastname=' +$("#lastname").val() + '&phonenumber=' +$("#phonenumber").val() + '&email=' +$("#email").val() + '&address1=' +$("#address_line1").val() + + '&address2=' +$("#address_line2").val() + + '&city=' +$("#city").val() + + '&state=' +$("#state").val() + + '&zipcode=' +$("#zipcode").val() + '&username=' +$('#username').val() + '&password=' +$('#password').val();
		$.ajax({
            type:'POST',
            url:'./register.php',
            data:datastr,
        ...
        /* $.post('./register.php',{firstname: $('#firstname').val(), lastname: $('#lastname').val(), phonenumber: $('#phonenumber').val(), email: $('#email').val(), address: $('#address').val(), username: $('#username').val(), password: $('#password').val()}, function(data){
			if(data=="1")
			{
